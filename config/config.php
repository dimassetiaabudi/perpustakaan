...
function deletePetugas($id)
{
  ...
  $deletePetugas = "DELETE FROM admin WHERE id = $id";

  mysqli_query($connection, $deletePetugas);
  return mysqli_affected_rows($connection);
}

function searchPetugas($keyword)
{
  // search member terdaftar || admin
  $searchMember = "SELECT * FROM admin WHERE nama_admin LIKE '%$keyword%'";
  return queryReadData($searchMember);
}
